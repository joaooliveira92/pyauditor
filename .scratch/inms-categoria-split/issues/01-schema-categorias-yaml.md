...
- [x] Modelos Pydantic (frozen/strict/extra=forbid, seguindo o padrão de `config/models.py`) para as
...
- [x] Função de loader (ex. `load_categorias(path: Path) -> ...`) que lê e valida um
...
- [x] `configs/MinC/categorias.yaml` preenchido com a tabela completa do §14.2 (INMS 1.1, 1.2, 1.6,
...
- [x] `configs/MTur/categorias.yaml` preenchido com a mesma tabela, mas com os literais de
...
- [x] 1.8 e 1.10 não têm entrada em nenhum dos dois arquivos (implícito — sem categoria)
