...
Cross-checagem por indicador: a tabela do RAS para **INMS 1.14** lista exatamente os mesmos seis serviços, na mesma ordem, do `inms-14.csv` — (a) File Server, (b) Telefonia, (c) Mensageria, (d) Servidores de impressão, (e) WI-FI, (f) Rede — com resultado agregado **98,49%**, batendo exatamente com o CSV e com o comentário do config (`configs/MinC/inms-1.14.yaml`) sobre correção a partir do RAS. **Confirma que os dados de INMS 1.14 já são reais**, apesar de o `observacao` de cada linha ainda dizer "Dados sinteticos" (rótulo obsoleto de antes da correção) — corrigido nesta sessão para citar o RAS/CITSmart como fonte.
