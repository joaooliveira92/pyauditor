...
- [Contrato CLI/dados/output por órgão](issues/04-cli-data-output-contract.md) — `--orgao {MinC,MTur,both}` implementado (default `MinC`); **`consolidate` (2.1)**: exige o par completo MinC+MTur (erro claro por órgão se falta), CLI agnóstica (sem `--orgao`), saída `reports/relatorio_<comp>_consolidado.xlsx`; merge na re-rodada por chave de linha `(indicador, órgão, competência)` — recomputa cálculo/valor sugerido, preserva colunas de decisão preenchidas (`Justificativa`/`Decisión Fiscal`/`Observación`), linha desaparecida ou decisão vs recomputado → aviso (nunca apagar/sobrescrever em silêncio); por-ativo (1.4/1.5/1.14) ficam uma linha por órgão, sem fundir, sem aportar glosa.
- [Mock da planilha consolidada](issues/05-consolidated-workbook-mock.md) — forma das 5 abas (CAPA/SERVICOS/INMS_BASE/GLOSAS/CALCULO_PAGAMENTO) aprovada pelo fiscal; formatos numéricos de `docs/styleguide.md` (moeda, percentual, borda de total) aplicados ao mock; prototype capturado na branch `prototype/consolidated-workbook-mock`, fora do master; color-coding por função de célula fica pendente para quando `consolidate` virar produção.
