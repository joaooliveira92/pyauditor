...
3. **Excel package (items 3, 6, 11, 12) — its own separate follow-up effort, split out from #2. DONE (2026-08-20), except the non-atomic-save half of item 11 (deliberately deferred).** Reasoning: most findings of any package (6 of 12 in P1-P3), holds the fiscal's irreplaceable hand-entered decision columns (the one thing this whole review's "must survive a hand-edited workbook" invariant exists to protect), and its bugs (dead `bold` var, cp1252 mangling, duplicate-header collision, no atomic saves) are self-contained to `excel/` — no shared root cause with #2's boundary-validation theme.
   ...
   - Item 11: `read_capa_fields`/`read_existing_decisions` calls in `cli/report.py`/`cli/consolidate.py` wrapped in `try/except Exception`, converting a corrupt/non-`.xlsx` file into an error Result instead of an uncaught `zipfile.BadZipFile`. Non-atomic `workbook.save()`/`write_historico` — **DONE (2026-08-20)**, no longer deferred: `src/pyauditor/atomic_write.py` writes to a temp file beside the target then `os.replace()`s it into place, wired into `bootstrap_capa`, `build_report`, `consolidate`'s workbook save, and `write_historico`. 5 new tests.
   ...
   - 16 new tests (`test_excel_consolidate.py`, `test_capa.py`, `test_cli_report.py`, `test_cli_consolidate.py`, `test_atomic_write.py`) covering each regression fixed. Full suite 235 passing, mypy/ruff clean on everything touched.
...
5. **P6 clarity/maintenance (items 21-26) — DONE (2026-08-20), on request ("continue the good work" / "work on the deferred issues"). All six items closed:**
   - Item 21 (orchestration half): `orchestration/summary.py`'s `_artifact_line` isinstance chain folded into a `dict[type, Callable]` table keyed the same way as `_result_for`'s `_RESULT_TYPE_FOR_COMMAND` — one type-discrimination point instead of two.
   - Item 21 (engine/rom half, initially deferred as speculative — done anyway): `CalculationStrategy` gained a `pool_numerator_denominator(memoria)` method alongside `calculate()`; `rom/summary.py` now delegates to `SHAPE_REGISTRY[shape].pool_numerator_denominator(...)` instead of a second shape-keyed if-chain. `as_float()` moved to `engine/strategies/_numbers.py`, shared by both layers. 13 new tests, including the previously-untested segmented_ratio/count_difference/external_catalog_sum/precomputed_table pooling branches (closing a P5 test gap too).
   - Item 22 (excel half): `_UNIT_BY_SHAPE`/`_new_sheet`/`_write` — byte-identical between `report.py` and `consolidate.py` — moved into the already-shared `excel/_style.py`.
   - Item 23 (initially deferred as non-urgent — done anyway): `cli_main`'s 5 per-subcommand bodies split into `_dispatch_measure`/`_dispatch_bootstrap`/`_dispatch_report`/`_dispatch_consolidate`/`_dispatch_run`, each independently callable with a hand-built `Namespace` — 2 new tests prove the actual point (unit-testable without argparse).
   - Item 25 (initially deferred as non-urgent — done anyway): `dependency_missing`'s report/consolidate branches now call `check_report_ready`/`check_consolidate_ready` directly (fully typed) instead of through `CHECKERS`'s erased `Callable[..., DependencyCheck]`; the registry stays in use for bootstrap/measure's zero-arg case, where the erasure was always harmless.
   - Item 26: the two `assert`s guarding genuinely external data (`pipeline.py`'s headerless-CSV check, `rom/render.py`'s three `memoria` shape checks) now raise `ValueError`/`TypeError` naming what's wrong, instead of being silently stripped under `python -O`. The two asserts guarding real internal invariants (Pydantic-validated config shape) were confirmed correct as `assert` and left alone.
   - All zero-behavior-change refactors verified by the full suite staying green at each step. Final state: 250 tests passing (up from 176 at the start of the whole review), coverage 92.5% (up from 88%), mypy/ruff clean on everything touched.

**This map's destination is reached, and every item on the punch list has a disposition — implemented or explicitly, reasoned-out-loud deferred.** Nothing left to decide.
