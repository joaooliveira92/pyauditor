...
2. **P1-P3 correctness/security/resilience (items 2, 4, 5, 7, 8, 9, 10, 13) — one cross-package "boundary validation + error handling" follow-up effort.** Scattered across `config`, `engine`, `orchestration`, `cli`, but the same fix *shape* throughout: validate/wrap at a boundary, raise an actionable domain exception with `from exc`. Bundling avoids re-deriving the same pattern five times as separate efforts.
3. **Excel package (items 3, 6, 11, 12) — its own separate follow-up effort, split out from #2.** Reasoning: most findings of any package (6 of 12 in P1-P3), holds the fiscal's irreplaceable hand-entered decision columns (the one thing this whole review's "must survive a hand-edited workbook" invariant exists to protect), and its bugs (dead `bold` var, cp1252 mangling, duplicate-header collision, no atomic saves) are self-contained to `excel/` — no shared root cause with #2's boundary-validation theme.
