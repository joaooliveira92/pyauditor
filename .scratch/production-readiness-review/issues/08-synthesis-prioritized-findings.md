...
5. **P6 clarity/maintenance (items 21-26) — deferred, left as loose reference in this ticket, no map. Partially picked up anyway (2026-08-20), on request ("continue the good work"):**
   - Item 21 (orchestration half): `orchestration/summary.py`'s `_artifact_line` isinstance chain folded into a `dict[type, Callable]` table keyed the same way as `_result_for`'s `_RESULT_TYPE_FOR_COMMAND` — one type-discrimination point instead of two. The engine/rom half (`SHAPE_REGISTRY` vs. `rom/summary.py`'s separate shape if-chain) stays deferred — informational/Low with no second concrete pain point beyond a hypothetical future 6th shape; forcing a fix now risks the speculative abstraction the project avoids.
   - Item 22 (excel half): `_UNIT_BY_SHAPE`/`_new_sheet`/`_write` — byte-identical between `report.py` and `consolidate.py` — moved into the already-shared `excel/_style.py`. Zero behavior change (full suite still green).
   ...
   - Items 23 (`main.py`'s `cli_main` dispatch split) and 25 (`CHECKERS`'s `Callable[..., DependencyCheck]` signature erasure) remain deferred — both were judged genuinely non-urgent by their own package reviews (23: "not urgent at current size"; 25: "not urgent given current call sites are already covered by a passing test suite"), and both are more invasive than the other P6 items for benefit that's still mostly speculative.
