...
Many teams use the [Ruff](https://docs.astral.sh/ruff/)
...
at build time with [ty](https://docs.astral.sh/ty/) in strict mode
(see `[tool.ty]` in `pyproject.toml`; ty's strict profile is stricter than the
old basedpyright strict mode). In most
cases, when feasible,
...
checking via ty (strict mode) in the build system. As static analysis
is
...
You can disable type checking on a line with the special comment `# ty:
ignore[<rule>]`, naming the specific ty rule that would otherwise fire.

ty's strict profile (the default; see `[tool.ty]` in `pyproject.toml`) honors
only `# ty: ignore[<rule>]` comments — `respect-type-ignore-comments = false`
turns the legacy `# type: ignore[...]` comments into ordinary comments, so they
...
Do not introduce new `# ty: ignore[<rule>]` without a comment explaining the
reason, and never use `Any` where a more precise type is available.
