...
class SegmentedRatioStrategy:
    def calculate(self, config: IndicatorConfig, rows: list[dict[str, str]]) -> CalculationResult:
        ...
        for category in calculation.categories:
            ...
            numerator_rows = filter_rows(denominator_rows, category.numerator_filter)
            ...
            category_shortfall = max(
                shortfall(result_pct, config.target.operator, config.target.value), 0.0
            )
            penalty = (category_shortfall / calculation.step_size_pct) * category.step_points
