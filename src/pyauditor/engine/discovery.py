    ...
    if isinstance(raw, dict) and 'indicator' not in raw:
        ...
        for key in raw:
            if isinstance(key, str) and key.strip().lower() in (
                ...
                raise ValueError(
                    f'{config_path}: chave {key!r} encontrada, esperado'
                    f" 'indicator:' — "
                    'typo no YAML, corrija a chave'
                )
        # also check if file looks like an indicator config (inms-*.yaml)
...
def _inject_orgao(
    ...
    if (
        config.scope.orgao == expected_orgao
        and config.scope.contract == desired_contract
    ):
        return config
...
def discover_config_files(
    ...
    is_shared = (
        config_dir.name == '_shared' or (config_dir / '_shared').exists()
    )
            ...
            if isinstance(raw, dict):
                for key in raw:
                    if isinstance(key, str) and key.strip().lower() in (
                        ...
                        raise ValueError(
                            f'{path}: chave {key!r} encontrada, esperado'
                            f" 'indicator:' — "
                            'typo no YAML'
                        )
        ...
        if expected_orgao is not None:
            ...
            if is_shared or config_dir.name == '_shared':
                ...
                raise ValueError(
                    ...
                    f' corresponde ao '
                    f'órgão solicitado {expected_orgao!r} — config no diretório'
                    f' errado'
                )
...
def discover_configs(
    ...
    return [
        config
        for _, _, config in discover_config_files(config_dir, expected_orgao)
    ]
