...
def load_summaries(roms_dir: Path) -> list[IndicatorSummary]:
    ...
    expected_orgao = (
        roms_dir.parent.name if roms_dir.parent.name in _ORGAOS else None
    )
    ...
    for summary_path in sorted(roms_dir.glob('*.json')):
        try:
            ...
            raise ValueError(
                f'sumário inválido em {summary_path}: {exc}'
            ) from exc
    return summaries
...
def read_valor_base(
    ...
    try:
        ...
        warnings.append(
            f'objetos.csv não encontrado em {data_dir} — glosa não calculada'
        )
        return None, ()
