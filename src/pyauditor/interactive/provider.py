...
class RichQuestionaryProvider:
    ...
    def ask_text(
        ...
        *,
        ...
        if not isinstance(answer, str):
            raise TypeError(
                "Questionary text prompt returned a non-string answer: "
    ...
    def ask_choice(
        ...
        *,
        ...
        if any(
            ...
            raise TypeError(
                "Single-choice prompt options must all be strings."
            )

        if default is not None and default not in normalized_choices:
            raise ValueError(
                f"Default choice {default!r} is not available."
            )
        ...
        if not isinstance(answer, str):
            raise TypeError(
                "Questionary select prompt returned a non-string answer: "
                f"{type(answer).__name__}."
            )
    ...
    def ask_multi_choice(
        ...
        for index, option in enumerate(normalized_choices):
            if len(option) != 4:
                raise ValueError(
                    f"Multiple-choice option {index} must contain exactly "
                    "four fields."
                )
        ...
        if len(values) != len(set(values)):
            raise ValueError(
                "Multiple-choice option values must be unique."
            )
        ...
        if unknown_values:
            raise ValueError(
                "Questionary checkbox returned unavailable values: "
    ...
    def confirm(
        ...
        *,
        ...
        if not isinstance(answer, bool):
            raise TypeError(
                "Questionary confirmation returned a non-boolean answer: "
                f"{type(answer).__name__}."
            )
