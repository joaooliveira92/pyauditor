...
def _organization_summary(
    ...
    if isinstance(report, ReportResult):
        ...
        measured_count = (
            len(measure.indicators) if isinstance(measure, MeasureResult) else 0
        )
        publicable = False
...
def _json_number(
    ...
    *,
    ...
    raise TypeError(
        f'{field} must be int, float, or Decimal; received'
...
def _errors_count(run_result: RunResult) -> int:
    ...
    return sum(
        1 for entry in run_result.state.commands if entry.status == 'error'
    )
...
def summary_json(
    run_result: RunResult, exit_code: int
) -> CompletionSummaryJson:
