...
_TRACKED_HEADER_NAMES: Final[frozenset[str]] = frozenset(
    {'Indicador', 'Órgão', *_DECISION_COLUMNS}
)


def _normalize_header(name: str) -> str:
    """Normaliza espaços/acentos/case para detectar renomeação leve
    (issue 08)."""
    import unicodedata
...
def _check_no_duplicate_headers(
    path: Path, header_row: Sequence[object]
) -> None:
...
def _check_renamed_headers(path: Path, header_row: Sequence[object]) -> None:
    ...
    for expected in _TRACKED_HEADER_NAMES:
        if expected not in present:
            ...
            # também tenta match por um caractere de diferença (ex. espaço extra
            # interno)
