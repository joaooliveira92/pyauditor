...
_PRODUCTION_COMMANDS: Final[frozenset[str]] = frozenset({"report", "consolidate"})
...
def _has_financial_issue(results: Sequence[_HasResult]) -> bool:
    ...
    return any(getattr(result, "glosa_calculada", True) is False for result in results)
...
def _has_publication_issue(results: Sequence[_HasResult]) -> bool:
    ...
    return any(getattr(result, "publicable", True) is False for result in results)
