...
- Capa criada: `capa_<orgao>.xlsx` (use
    [Crie a capa do contrato](bootstrap-capa.md)).
...
- As abas esperadas estão presentes — veja
...
- `CADASTROS` é preenchido a partir dos configs; se o `--config-dir` falhar ao
    carregar, as abas `CADASTROS`/`EVIDENCIAS` são omitidas com
    aviso (o restante do relatório é gerado).
