...
> **Preenchimento automatizado (spec `competencia-cli-equipe`)**: competência e os períodos de aferição são derivados do argumento `--competência` da CLI — nunca hand-fill. Fiscal técnico, fiscal requisitante, fiscal administrativo e
