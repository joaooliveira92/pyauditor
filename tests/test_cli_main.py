...
def test_cli_main_happy_path(tmp_path: Path) -> None:
    ...
    for p in (cfg, data, out):
        p.mkdir()
    with patch(
        'pyauditor.cli.main.run_measure',
...
def test_cli_main_bootstrap_dispatches_with_capa_path(tmp_path: Path) -> None:
    ...
    with patch(
        'pyauditor.cli.main.run_bootstrap',
        return_value=SimpleNamespace(status='done'),
    ) as m:
        code = cli_main(
            [
...
def test_cli_main_measure_writes_a_traceable_run_log(tmp_path: Path) -> None:
    ...
    assert code == 1
    log_files = sorted(
        (out / 'MinC' / '2026-06').glob('pyauditor-measure-2026-06-*.log')
    )
...
def test_cli_main_measure_resolves_shared_manifest_when_both_exist(
    tmp_path: Path,
) -> None:
...
def test_cli_main_report_dispatches_with_output_filename(
    tmp_path: Path,
) -> None:
    ...
    with (
        ...
        assert (
            m.call_args.kwargs['output_path']
            == out_dir / 'relatorio_2026-06_MinC.xlsx'
        )
...
def test_dispatch_measure_is_callable_directly_without_argparse(
    tmp_path: Path,
) -> None:
...
def test_dispatch_bootstrap_is_callable_directly_without_argparse(
    tmp_path: Path,
) -> None:
    capa_path = tmp_path / 'capa.csv'
    args = build_parser().parse_args(
        ['bootstrap', '--capa-path', str(capa_path)]
    )
    with patch(
        'pyauditor.cli.main.run_bootstrap',
        return_value=SimpleNamespace(status='done'),
...
def test_cli_main_consolidate_final_month_reaches_run_consolidate(
    tmp_path: Path,
) -> None:
...
def test_cli_main_no_args_without_tty_exits_2(
...
def test_cli_main_no_args_with_tty_runs_interactive(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
