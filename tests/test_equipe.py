...
class TestReadEquipe:
    def test_mapeia_as_quatro_funcoes_normalizando_caixa_e_acento(
        self, tmp_path: Path
    ) -> None:
        path = _escreve(
            tmp_path,
            f'{_CABECALHO}\n'
            'Gestor do Contrato,João Antônio Carvalho Monteiro de '
            'Oliveira,1499628\n'
    ...
    def test_celula_formato_nome_siape(self, tmp_path: Path) -> None:
        ...
        assert (
            read_equipe(path).cell('Fiscal técnico') == 'Maria Souza (1111111)'
        )
    ...
    def test_substituto_nao_mapeia_para_campo_da_capa(
        self, tmp_path: Path
    ) -> None:
        path = _escreve(
            tmp_path,
    ...
    def test_substituto_sem_hifen_tambem_reconhecido(
        ...
        equipe = read_equipe(path)
        assert (
            equipe.responsaveis_fields()['Fiscal técnico'] == 'Titular Um (1)'
        )
    ...
    def test_bom_utf8_sig_lido_corretamente(self, tmp_path: Path) -> None:
        ...
        assert (
            read_equipe(path).responsaveis_fields()['Fiscal técnico']
            == 'Maria Souza (1)'
        )
    ...
    def test_arquivo_ausente_e_decisao_do_chamador(
        self, tmp_path: Path
    ) -> None:
...
class TestWarnings:
    ...
    def test_responsaveis_fields_devolve_so_presentes(
        self, tmp_path: Path
    ) -> None:
        path = _escreve(
            tmp_path, f'{_CABECALHO}\nFiscal técnico,Maria Souza,1\n'
