...
def test_inms_base_row_matches_summary_and_leaves_fiscal_columns_blank() -> (
    None
):
    summaries = [
        _summary(
    ...
    row = {
        header[i]: sheet.cell(row=2, column=i + 1).value
...
def test_group_tabs_only_contain_their_indicators() -> None:
    ...
    n1_codes = [
        n1_sheet.cell(row=r, column=1).value
        for r in range(2, n1_sheet.max_row + 1)
    ]
...
def test_split_derived_summary_lands_on_its_own_categoria_tab() -> None:
    ...
    assert (
        'INMS 1.01' not in n1_codes
    )  # would be primary_group's first-listed tab
...
def test_indicator_outside_group_tabs_only_appears_in_inms_base() -> None:
    ...
    base_sheet = workbook[INMS_BASE_SHEET]
    assert (
        base_sheet.cell(row=2, column=5).value == 'INMS 1.08'
    )  # Código INMS column
...
def test_inms_base_never_synthesizes_a_consolidado_row() -> None:
    ...
    base_sheet = workbook[INMS_BASE_SHEET]
    orgaos_in_base = [
        str(base_sheet.cell(row=r, column=7).value)
        for r in range(2, base_sheet.max_row + 1)
    ]
    ...
    row = {
        header[i]: glosas_sheet.cell(row=2, column=i + 1).value
        for i in range(len(header))
    }
...
def test_cadastros_sheet_populated_from_configs() -> None:
    ...
    codes = [
        sheet.cell(row=r, column=1).value for r in range(2, sheet.max_row + 1)
    ]
...
def test_evidencias_sheet_has_dropdown_validations() -> None:
    ...
    workbook = build_report_workbook(
        '2026-06', [_summary('INMS-1.1', 'INMS 1.1')], configs=configs
    )
    sheet = workbook[EVIDENCIAS_SHEET]
