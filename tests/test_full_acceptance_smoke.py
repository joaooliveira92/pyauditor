...
@pytest.mark.skipif(not INPUT_DIR.exists(), reason="production data (/input) not present locally")
...
def test_acceptance_test_matches_real_data(config: IndicatorConfig) -> None:
    assert config.acceptance_test is not None, f"{_config_id(config)}: config has no acceptance_test"
    ...
    assert calc.result_pct == pytest.approx(expected.result_pct, abs=0.01), (
        f"{label}: result_pct — expected {expected.result_pct}, got {calc.result_pct}"
    )
    assert calc.conforms == expected.conforms, (
        f"{label}: conforms — expected {expected.conforms}, got {calc.conforms}"
    )
    assert calc.penalty_points == pytest.approx(expected.penalty_points, abs=0.01), (
        f"{label}: penalty_points — expected {expected.penalty_points}, got {calc.penalty_points}"
    )

    if isinstance(expected, RatioAcceptanceExpected):
        assert calc.memoria.get("numerator") == pytest.approx(expected.numerator), (
            f"{label}: numerator — expected {expected.numerator}, got {calc.memoria.get('numerator')}"
        )
        assert calc.memoria.get("denominator") == pytest.approx(expected.denominator), (
            f"{label}: denominator — expected {expected.denominator}, got {calc.memoria.get('denominator')}"
        )

    elif isinstance(expected, SegmentedRatioAcceptanceExpected):
        categories = calc.memoria.get("categories")
        assert isinstance(categories, list), f"{label}: expected categories in memoria, found none"
        for expected_category, actual_category in zip(expected.categories, categories, strict=True):
            assert actual_category.get("name") == expected_category.name, (
                f"{label}/{expected_category.name}: category name mismatch"
            )
            assert actual_category.get("numerator") == expected_category.numerator, (
                f"{label}/{expected_category.name}: numerator — expected {expected_category.numerator}, "
                f"got {actual_category.get('numerator')}"
            )
            assert actual_category.get("denominator") == expected_category.denominator, (
                f"{label}/{expected_category.name}: denominator — expected {expected_category.denominator}, "
                f"got {actual_category.get('denominator')}"
            )
            assert actual_category.get("penalty_points") == pytest.approx(expected_category.penalty_points), (
                f"{label}/{expected_category.name}: penalty_points — expected {expected_category.penalty_points}, "
        ...
        assert calc.memoria.get("QRC") == expected.qrc, (
            f"{label}: QRC — expected {expected.qrc}, got {calc.memoria.get('QRC')}"
        )
        assert calc.memoria.get("QCSI") == expected.qcsi, (
            f"{label}: QCSI — expected {expected.qcsi}, got {calc.memoria.get('QCSI')}"
        )
        assert calc.memoria.get("CNI") == expected.cni, (
            f"{label}: CNI — expected {expected.cni}, got {calc.memoria.get('CNI')}"
        )
