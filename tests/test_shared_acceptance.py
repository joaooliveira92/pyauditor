"""Ticket 03 — `acceptance_test` migrou de `configs/_shared/` para `tests/acceptance/`.
...
def test_shared_measure_matches_acceptance_fixture(orgao: str) -> None:
    ...
    configs = {c.indicator.id: c for c in discover_configs(SHARED_DIR, expected_orgao=orgao)}
    ...
    strict_orgaos = {"MinC"}
    strict_iids = {"INMS-01", "INMS-02", "INMS-06", "INMS-07", "INMS-11", "INMS-12"}
    ...
    for iid, expected_raw in acceptance.items():
        ...
        try:
            ...
            # Degenerado / sem coluna — aceita skip. Só falha se for MinC+strict.
            if orgao not in strict_orgaos or iid not in strict_iids:
                continue
            raise AssertionError(f"{orgao}/{iid} falhou inesperado: {exc}") from exc
        if orgao not in strict_orgaos or iid not in strict_iids:
