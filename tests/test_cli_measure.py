...
def test_run_measure_categoria_missing_dataset_marks_not_activated(tmp_path: Path) -> None:
    ...
    assert result.status == "done"
    assert len(result.indicators) == 2
    assert all(o.not_activated and not o.hard_failure for o in result.indicators)
...
def test_run_measure_categoria_missing_grupo_executor_column(tmp_path: Path) -> None:
    ...
    assert result.status == "error"
    assert any("não tem coluna 'Grupo_executor'" in (o.error or "") for o in result.indicators)
...
""",
    ...
    assert result.status == "done"
    assert any("['N0'] sem correspondência — valores não encontrados no CSV" in w for w in result.warnings)
...
class _FailingQualityGateRunner:
    """Stub que derruba na execução para cobrir o ramo de exceção por categoria."""
...
def test_run_measure_categoria_outros_warns_when_standalone(tmp_path: Path) -> None:
    ...
    assert result.status == "done"
    assert any(
        "categoria outros: 1 linha(s) não classificada(s) em nenhuma categoria" in w
...
def test_run_measure_no_configs_found_is_error(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "roms"
    config_dir.mkdir(parents=True)
    (config_dir / "datasets.yaml").write_text("manifests: []\n", encoding="utf-8")
...
def test_run_measure_mkdir_failure_is_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config_dir, data_dir, output_dir = _write_whole_indicator_empty_window(tmp_path)
...
def test_run_measure_categorias_fallback_parent_orgao_dir(tmp_path: Path) -> None:
    ...
    data_dir = tmp_path / "input"
    output_dir = tmp_path / "roms"
    (configs / "_shared" / "inms-01.yaml").write_text(_CATEGORIA_CONFIG_YAML, encoding="utf-8")
    (configs / "MinC").mkdir(parents=True)
    (configs / "MinC" / "categorias.yaml").write_text(_CATEGORIAS_YAML, encoding="utf-8")
    competencia_data_dir = data_dir / "2026" / "06"
    competencia_data_dir.mkdir(parents=True)
    (competencia_data_dir / "inms-01.csv").write_text(_CATEGORIA_RAW_CSV, encoding="utf-8")

    result = run_measure(
        "2026-06", configs / "_shared", data_dir, output_dir, expected_orgao="MinC"
    )
