...
def _fixture(
    tmp_path: Path, csv_body: str = _CSV_OK
...
def test_no_configs_returns_1(tmp_path: Path) -> None:
    ...
    config_dir.mkdir()
    assert (
        run_measure('2026-06', config_dir, tmp_path, tmp_path).status == 'error'
    )


def test_invalid_competencia_rejected(tmp_path: Path) -> None:
    assert (
        run_measure('2026/06', tmp_path, tmp_path, tmp_path).status == 'error'
    )
    assert (
        run_measure('../../etc', tmp_path, tmp_path, tmp_path).status == 'error'
    )
    assert (
        run_measure('2026-13-extra', tmp_path, tmp_path, tmp_path).status
        == 'error'
    )
...
def test_mkdir_oserror_returns_1(tmp_path: Path) -> None:
    ...
    with patch.object(Path, 'mkdir', side_effect=OSError('ro')):
        assert (
            run_measure('2026-06', config_dir, data_dir, tmp_path).status
            == 'error'
        )
...
def test_measure_exception_continues(tmp_path: Path) -> None:
    ...
    missing = next(
        i for i in code.indicators if i.rom_path.name == 'INMS-MISSING.md'
    )
...
def test_missing_equipe_warns_but_does_not_fail(tmp_path: Path) -> None:
    ...
    code = run_measure(
        '2026-06', config_dir, data_dir, out_dir, equipe_path=missing_equipe
    )
...
def test_equipe_fields_reach_render_rom_and_malformed_degrades(
    tmp_path: Path,
) -> None:
    config_dir, data_dir, out_dir = _fixture(tmp_path)
    equipe_path = tmp_path / 'equipe.csv'
    ...
    assert code.status == 'done'
    rom_text = (out_dir / '2026-06' / 'INMS-TEST.md').read_text(
        encoding='utf-8'
    )
    assert 'Gestor do contrato: Beltrano (7654321)' in rom_text

    # Malformado é dado incompleto, não falha — Responsáveis voltam ao
    # placeholder.
