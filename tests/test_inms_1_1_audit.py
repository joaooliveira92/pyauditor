...
_INMS_01_RAW_CSV_ENRIQUECIDO = (
    ...
    'S;N1;Fulano\n'
    '2;Instalação de software;01/06/2026 09:00;01/06/2026 11:00;02/06/2026 '
    '09:00;'
    'N;N1;Fulano\n'
...
def _write_fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (data_dir / 'inms-01.csv').write_text(
        _INMS_01_RAW_CSV_ENRIQUECIDO, encoding='utf-8'
    )
...
def test_enriched_sheet_is_used_when_raw_csv_has_detail_columns(
    ...
    assert (
        sheet['A1'].value == 'INMS 1.1 – Incidentes atendidos dentro do prazo'
    )
...
def test_enriched_sheet_sanitizes_formula_injection_from_raw_csv(
    tmp_path: Path,
) -> None:
    config_dir, data_dir = _write_fixture(tmp_path)
    malicious_csv = (
        ...
        '+Atividade maliciosa;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:30;'
        'S;N1;-Fulano\n'
    )
    ...
    assert (
        sheet['R2'].value == '\'=HYPERLINK("https://exemplo.invalid","clique")'
    )
    assert sheet['T2'].value == "'+Atividade maliciosa"
    assert sheet['Y2'].value == "'-Fulano"


def test_enriched_sheet_flags_invalid_and_inconsistent_dates(
    tmp_path: Path,
) -> None:
    config_dir, data_dir = _write_fixture(tmp_path)
    csv_with_bad_dates = (
        ...
        '1;Reset de senha;31/02/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:30;S;N1;Fulano\n'
        ...
        '3;Restart;01/06/2026 12:00;01/06/2026 14:00;01/06/2026 '
        '10:00;N;N1;Fulano\n'
        # Linha 5: tudo válido.
        '4;Acesso;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:00;S;N1;Fulano\n'
...
def test_zero_denominator_formulas_are_guarded_against_div0(
    tmp_path: Path,
) -> None:
    ...
    assert 'IF(B' in formula(f'E{total_row}') and 'Sem ocorrências' in formula(
        f'E{total_row}'
    )
    ...
    divergentes_label_row = next(
        ...
        for cell in row
        if cell.value
        == 'Registros com limite ITSM superior ao contratual bruto:'
    )
...
def test_section_3_narrativa_reflects_actual_margin_sign(
    tmp_path: Path,
) -> None:
...
@pytest.mark.parametrize(
    'raw_value', ['s', 'n', ' S ', ' N ', 'Sim', 'Não', 'N/A', '']
)
def test_no_prazo_invalid_or_variant_values(
...
def test_iap_denominator_uses_rows_not_counta(tmp_path: Path) -> None:
    ...
    csv_with_blank_and_duplicate_id = (
        ...
        'Solicitacao;Atividades;DataHoraSolicitacao;DataHoraLimite;DataHoraFim;'
        'No prazo;Grupo_executor;TecnicoExecutor\n'
        ';Reset de senha;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:30;S;N1;Fulano\n'
        '1;Instalação;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:00;S;N1;Fulano\n'
        '1;Acesso;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:00;N;N1;Fulano\n'
    )
    (data_dir / 'inms-01.csv').write_text(
        csv_with_blank_and_duplicate_id, encoding='utf-8'
    )
    categorias_file = load_categorias(config_dir / 'categorias.yaml')
    output_path = tmp_path / 'sintetico.xlsx'
    periodo = PeriodoAfericao(date(2026, 6, 1), date(2026, 6, 30))
...
def test_section_8_exposes_rejected_record_count(tmp_path: Path) -> None:
    ...
    csv_with_bad_dates = (
        ...
        '09:30;S;N1;Fulano\n'
        '2;Instalação;01/06/2026 08:00;01/06/2026 10:00;;S;N1;Fulano\n'
        '3;Acesso;01/06/2026 08:00;01/06/2026 10:00;01/06/2026 '
        '09:00;S;N1;Fulano\n'
    )
    (data_dir / 'inms-01.csv').write_text(csv_with_bad_dates, encoding='utf-8')
    categorias_file = load_categorias(config_dir / 'categorias.yaml')
    ...
    rejeitados_cell = next(
        ...
        and cell.value.startswith('Registros excluídos da média')
    )
    assert isinstance(rejeitados_cell.row, int)
    rejeitados_row = rejeitados_cell.row
    assert (
        sheet.cell(row=rejeitados_row, column=2).value
        == '=COUNTIF($AG$2:$AG$4,"")'
    )
...
def test_atraso_medio_uses_itsm_calculated_column(tmp_path: Path) -> None:
    ...
    atraso_cell = next(
        ...
        for cell in row
        if isinstance(cell.value, str)
        and cell.value.startswith('Atraso médio dos registros')
    )
...
def test_limite_itsm_menor_que_contratual_bruto_nao_e_sinalizado(
    tmp_path: Path,
) -> None:
    ...
    (data_dir / 'inms-01.csv').write_text(
        csv_com_limite_itsm_mais_rigido, encoding='utf-8'
    )
    categorias_file = load_categorias(config_dir / 'categorias.yaml')
    output_path = tmp_path / 'sintetico.xlsx'
    periodo = PeriodoAfericao(date(2026, 6, 1), date(2026, 6, 30))
    ...
    sample_row = next(
        ...
        and cell.value.startswith(
            'Nenhum limite ITSM superior ao contratual bruto'
        )
    )
    ...
    categorias_file = CategoriasFile(
        categorias={
            'ATENDIMENTO_N1': CategoriaConfig(
                label='Atendimento Remoto aos Usuários',
                inms={
                    '1.1': GrupoExecutorMode(
                        mode='grupo_executor', in_values=['N1']
                    )
                },
            ),
...
def test_write_sheet_twice_in_same_workbook_uses_unique_table_names(
    tmp_path: Path,
) -> None:
...
def test_section_5_nivel_rows_use_body_font_consistently(
    tmp_path: Path,
) -> None:
...
def test_categoria_sem_nivel_configurado_usa_sem_nivel_sentinela(
    tmp_path: Path,
) -> None:
