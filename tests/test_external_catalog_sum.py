...
def test_external_catalog_sum_applies_max_points_wins_dedup(tmp_path: Path) -> None:
    config_yaml = """
indicator:
  id: INMS-TEST-1.8
...
"""
    ...
    assert [o["occurrence_id"] for o in occurrences] == ["OC-001", "OC-002", "OC-003"]
    assert occurrences[2]["catalog_id"] == "OD-02"  # maior pontuação vence sobre OD-03
    assert occurrences[2]["pontos"] == 5000


def test_external_catalog_sum_conforms_when_no_occurrences_matched(tmp_path: Path) -> None:
    config_yaml = """
indicator:
