...
def test_glosa_consumes_saldo_anterior() -> None:
    ...
    result = compute_glosa(total_points=20_000.0, valor_base=100_000.0, saldo_anterior_pct=15.0)
...
def test_glosa_raw_pct_matches_percentual_plus_saldo_rolado() -> None:
    ...
    assert result.raw_pct == pytest.approx(result.percentual_ajuste + result.saldo_rolado_pct)
...
def test_houve_reincidencia_below_threshold() -> None:
    ...
    assert houve_reincidencia(historico, "2026-06", teto_atingido_mes_atual=True) is False
...
def test_houve_reincidencia_three_times_in_six_months() -> None:
    ...
    assert houve_reincidencia(historico, "2026-06", teto_atingido_mes_atual=True) is True
