...
def test_resolve_config_dir_prefers_shared_when_dir_exists(
    tmp_path: Path,
) -> None:
...
def test_resolve_config_dir_falls_back_to_orgao_without_shared(
    tmp_path: Path,
) -> None:
    (tmp_path / _ORG).mkdir()
...
def test_resolve_manifest_path_prefers_shared_when_both_exist(
    tmp_path: Path,
) -> None:
    (tmp_path / '_shared').mkdir()
    (tmp_path / _ORG).mkdir()
    ...
    assert (
        resolve_manifest_path(tmp_path, _ORG)
        == tmp_path / '_shared' / 'datasets.yaml'
    )


def test_resolve_manifest_path_falls_back_to_orgao_without_shared(
    tmp_path: Path,
) -> None:
    (tmp_path / _ORG).mkdir()
    (tmp_path / _ORG / 'datasets.yaml').write_text(
    ...
    assert (
        resolve_manifest_path(tmp_path, _ORG)
        == tmp_path / _ORG / 'datasets.yaml'
    )


def test_load_manifest_for_uses_shared_manifest_when_both_exist(
    tmp_path: Path,
) -> None:
...
def test_load_manifest_for_returns_none_when_no_manifest(
    tmp_path: Path,
) -> None:
    (tmp_path / _ORG).mkdir()
...
def test_load_manifest_for_requires_manifest_in_resolved_config_dir(
    tmp_path: Path,
) -> None:
