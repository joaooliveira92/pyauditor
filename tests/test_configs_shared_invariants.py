...
# Campos que DEVEM ser idênticos entre órgãos. `source` é contrato de dataset (mesmo CSV lógico),
# `quality_gates`/`calculation`/`target`/`penalty` são regra do Anexo D.
INVARIANT_TOP_LEVEL_KEYS = ("source", "quality_gates", "calculation", "target", "penalty")

# `scope` e `acceptance_test` divergem por design (órgão/contrato e snapshot da competência).
...
def test_shared_invariants_between_orgaos() -> None:
    ...
    if len(stems) < 14:
        ...
        assert not failures_ss, "Invariantes via _shared divergiram:\n" + "\n".join(failures_ss)
        return
    ...
    for fname in stems:
        ...
        # `indicator.name` e `contractual_id` também são invariantes (mesmo indicador)
...
def test_no_tracked_derived_configs() -> None:
    """Garante que derivados `inms-*.*.yaml` não estão trackeados (gitignored)."""
    import subprocess

    result = subprocess.run(
        ["git", "ls-files", "--cached", "configs/MinC/inms-*.*.yaml", "configs/MTur/inms-*.*.yaml"],
        capture_output=True,
        text=True,
        cwd=str(REPO_ROOT),
    )
    tracked = [line for line in result.stdout.splitlines() if line.strip()]
    assert not tracked, f"Derivados trackeados encontrados (deveriam ser gitignored): {tracked}"
...
def test_derived_configs_are_gitignored() -> None:
    ...
    result = subprocess.run(
        ["git", "check-ignore", "-v", "configs/MinC/inms-01.ATENDIMENTO_N1.yaml"],
        capture_output=True,
        text=True,
        cwd=str(REPO_ROOT),
    )
    assert result.returncode == 0, "gitignore deveria cobrir configs/*/inms-*.*.yaml"
