...
def test_identificacao_without_config_path_shows_unavailable(
    tmp_path: Path,
) -> None:
    config_path = _write_ratio_fixture(tmp_path, resultado=100.0)
    config = load_config(config_path)
...
def test_competencia_periodo_kwargs_e_capa_so_responsaveis(
    tmp_path: Path,
) -> None:
    ...
    rom = render_rom(
        result, capa_fields=capa_fields, competencia='2026-06', periodo=periodo
    )
...
def test_measure_emite_warn_de_janela_vazia_e_info_descarte(
    tmp_path: Path,
) -> None:
...
def test_ressalva_interpretativa_shown_with_correct_readings(
    tmp_path: Path,
) -> None:
...
def test_footer_note_present(tmp_path: Path) -> None:
    ...
    assert (
        'Responsáveis refletem o estado da capa no momento em que este ROM foi '
        'gerado' in rom
    )
...
def test_render_unknown_shape_raises_actionable_value_error(
    tmp_path: Path,
) -> None:
    ...
    bogus = result.config.model_copy(
        update={
            'calculation': config.calculation.model_copy(
                update={'shape': 'bogus_shape'}
            )
        }
    )
...
def test_render_combined_rom_stacks_both_orgaos(tmp_path: Path) -> None:
    ...
    result_minc = measure(
        load_config(minc_path), data_dir=tmp_path, config_path=minc_path
    )
    result_mtur = measure(
        load_config(mtur_path), data_dir=tmp_path, config_path=mtur_path
    )

    rom = render_combined_rom(result_minc, result_mtur)

    assert rom.startswith(
...
def test_linhas_fora_do_periodo_aparece_somente_com_filtro(
    tmp_path: Path,
) -> None:
...
def _write_orgao_fixture(
    tmp_path: Path, orgao: str, *, resultado: float
) -> Path:
    yaml = _RATIO_YAML
    if orgao != 'MinC':
