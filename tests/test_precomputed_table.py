...
from pyauditor.config.models import IndicatorConfig, PrecomputedTableAcceptanceExpected
...
@pytest.mark.skipif(not (INPUT_DIR / "inms-04.csv").exists(), reason="dados locais ausentes")
...
def test_precomputed_table_skips_empty_rows_and_parses_comma_decimals(tmp_path: Path) -> None:
...
"""
    ...
    assert calc.result_pct == pytest.approx((198.5 + 190.2) / 400.0 * 100.0, abs=0.01)
    assert calc.penalty_points == pytest.approx(1000.0)
    assert calc.conforms is False


def test_precomputed_table_points_mode_reads_excess_over_meta(tmp_path: Path) -> None:
