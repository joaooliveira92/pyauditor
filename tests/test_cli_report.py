...
def test_sidecar_legado_sem_campos_de_periodo_carrega_com_defaults(
    tmp_path: Path,
) -> None:
...
def test_run_report_persists_glosa_historico(tmp_path: Path) -> None:
    ...
    _write_summary_with_points(
        roms_dir, '2026-06', 'INMS-1.1', 'INMS 1.1', 40_000.0
    )
    ...
    historico = json.loads(
        (roms_dir / 'glosa_historico.json').read_text(encoding='utf-8')
    )
...
def test_run_report_next_competencia_consumes_rollover(tmp_path: Path) -> None:
    ...
    # Julho sem nenhuma penalidade nova â€” sÃ³ o saldo rolado de junho (10
    # p.p.)
    ...
    assert (
        glosas_sheet.cell(row=2, column=3).value == 10.0
    )  # saldo recebido do mÃªs anterior
    assert (
        glosas_sheet.cell(row=2, column=4).value == 10.0
...
def test_run_report_converts_unexpected_exception_to_error_result(
    tmp_path: Path,
) -> None:
    comum = _scaffold_capas(tmp_path)
    roms_dir = tmp_path / 'roms'
    ...
    with patch(
        'pyauditor.cli.report.build_report', side_effect=ValueError('boom')
    ):
        result = run_report(
            '2026-06',
            comum,
...
def test_run_report_os_error_message_has_actionable_hint(
    tmp_path: Path,
) -> None:
    ...
    with patch(
        'pyauditor.cli.report.build_report',
...
def test_run_report_names_the_offending_file_for_a_malformed_summary(
    tmp_path: Path,
) -> None:
    comum = _scaffold_capas(tmp_path)
    roms_dir = tmp_path / 'roms'
    _write_summary(roms_dir, '2026-06', 'INMS-1.1', 'INMS 1.1')
    (roms_dir / '2026-06' / 'INMS-1.1.json').write_text(
...
def test_run_report_capa_do_orgao_exibe_derivados_da_cli_e_equipe(
    tmp_path: Path,
) -> None:
...
def test_run_report_is_regenerated_not_cumulative(tmp_path: Path) -> None:
    ...
    sheet = workbook[INMS_BASE_SHEET]
    codes = [
        str(sheet.cell(row=r, column=5).value)
...
def test_run_report_reads_valor_base_from_objetos_for_glosas(
    tmp_path: Path,
) -> None:
    comum = _scaffold_capas(tmp_path)
    roms_dir = tmp_path / 'roms'
    _write_summary(
        roms_dir, '2026-06', 'INMS-1.1', 'INMS 1.1'
    )  # penalty_points=222.14
    output_path = tmp_path / 'reports' / 'relatorio.xlsx'
    ...
    assert glosas_sheet.cell(row=2, column=6).value == round(
        461063.58 * 0.22214 / 100, 2
    )


def test_run_report_without_objetos_marks_glosa_nao_calculada(
    tmp_path: Path,
) -> None:
    comum = _scaffold_capas(tmp_path)
...
def test_run_report_missing_fiscais_is_rascunho_nao_publicavel(
    tmp_path: Path,
) -> None:
    ...
    assert (
        result.publicable is False
    )  # capa_MinC.csv recém-bootstrapped: fiscais vazios
...
def test_run_report_valor_mensal_zero_is_glosa_calculada(
    tmp_path: Path,
) -> None:
    ...
    glosas_sheet = workbook[GLOSAS_SHEET]
    assert (
        glosas_sheet.cell(row=2, column=5).value == 0.0
    )  # Valor-base — numérico, não vazio
