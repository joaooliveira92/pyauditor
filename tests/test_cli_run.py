...
def test_run_run_executes_pipeline_and_returns_4_for_unfilled_capa(
    tmp_path: Path,
) -> None:
    (tmp_path / 'configs' / 'MinC').mkdir(parents=True)
    (tmp_path / 'input' / 'MinC' / '2026' / '06').mkdir(parents=True)
...
def test_run_run_default_resumes_and_does_not_rerun_done_steps(
    tmp_path: Path,
) -> None:
    ...
    assert (
        second == 4
    )  # inalterado — measure/report já "done", não reprocessados
...
def test_run_run_isolates_orgao_failure_both_orgaos(tmp_path: Path) -> None:
    ...
    assert not (
        paths['report_dir'] / 'relatorio_2026-06_consolidado.xlsx'
    ).exists()
