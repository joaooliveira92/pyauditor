...
class TestMesBounds:
    def test_mes_simples(self) -> None:
        assert month_bounds('2026-06') == PeriodoAfericao(
            date(2026, 6, 1), date(2026, 6, 30)
        )

    def test_dezembro_fecha_no_ultimo_dia_do_ano(self) -> None:
        assert month_bounds('2025-12') == PeriodoAfericao(
...
class TestFilterPeriodo:
    ...
    def test_mistos_mantem_janela_e_conta_descartes(self) -> None:
        ...
        resultado = filter_periodo(
            linhas, period_column=COLUNA, periodo=self.JUNHO
        )
    ...
    def test_formato_mes_yyyy_mm(self) -> None:
        ...
        assert [linha[COLUNA] for linha in resultado.linhas_na_janela] == [
            '2026-06'
        ]
        assert resultado.dropped_out_of_period == 1
    ...
    @pytest.mark.parametrize(
        'valor', ['', '   ', 'ontem', '2026/06', '2026-06-15']
    )
    def test_sem_data_legivel_default_segue_para_gates(
        self, valor: str
    ) -> None:
    ...
    def test_coluna_chave_ausente_tratada_como_sem_data(self) -> None:
        ...
        default = filter_periodo(
            linhas, period_column=COLUNA, periodo=self.JUNHO
        )
        assert len(default.linhas_na_janela) == 1
        strict = filter_periodo(
            linhas, period_column=COLUNA, periodo=self.JUNHO, strict=True
        )
...
class TestRequirePeriodColumn:
    def test_retorna_coluna_quando_declarada(self) -> None:
        assert (
            require_period_column('Data da medição', config_path='x.yaml')
            == 'Data da medição'
        )
...
class TestMensagens:
    ...
    def test_empty_window_message(self) -> None:
        assert empty_window_message(self.JUNHO) == (
            'nenhuma linha no período 01/06/2026–30/06/2026 — o arquivo '
            'corresponde à competência?'
        )

    def test_discard_message_somente_fora(self) -> None:
        assert (
            discard_message(3, 0, strict=False)
            == '3 linha(s) fora do período descartada(s)'
        )
    ...
    def test_discard_message_so_sem_data_sob_strict(self) -> None:
        assert (
            discard_message(0, 4, strict=True)
            == '4 linha(s) sem data legível descartada(s)'
        )
