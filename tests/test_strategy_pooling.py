...
def test_segmented_ratio_pools_by_summing_categories() -> None:
    ...
    numerator, denominator = (
        SegmentedRatioStrategy().pool_numerator_denominator(memoria)
    )
...
def test_segmented_ratio_pooling_tolerates_malformed_categories() -> None:
    assert SegmentedRatioStrategy().pool_numerator_denominator(
...
def test_count_difference_pools_qcsi_as_numerator_and_qrc_as_denominator() -> (
    None
):
    numerator, denominator = (
        CountDifferenceStrategy().pool_numerator_denominator(
            {'QRC': 10, 'QCSI': 8, 'CNI': 2}
        )
    )
...
def test_external_catalog_sum_has_no_pooled_numerator_denominator() -> None:
    assert ExternalCatalogSumStrategy().pool_numerator_denominator(
        {'total_points': 5}
    ) == (
        None,
        None,
...
def test_precomputed_table_has_no_pooled_numerator_denominator() -> None:
    assert PrecomputedTableStrategy().pool_numerator_denominator(
        {'categories': []}
    ) == (
        None,
        None,
