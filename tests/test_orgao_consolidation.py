...
def test_consolidated_conforms_matches_target() -> None:
    minc = _summary(
        'INMS 1.1', 'MinC', numerator=99, denominator=100, target_value=98.0
    )
    mtur = _summary(
        'INMS 1.1', 'MTur', numerator=99, denominator=100, target_value=98.0
    )
...
def test_consolidated_conforms_tolerates_float_rounding_at_the_target() -> None:
    ...
    minc = _summary(
        'INMS 1.1', 'MinC', numerator=0.1, denominator=0.3, target_value=100.0
    )
    mtur = _summary(
        'INMS 1.1', 'MTur', numerator=0.7, denominator=0.5, target_value=100.0
    )
    ...
    assert (
        consolidated.result_pct < 100.0
    )  # confirms the float-rounding premise
    assert consolidated.conforms is True
...
def test_consolidated_synthetic_row_carries_no_penalty_points() -> None:
    ...
    minc = _summary(
        'INMS 1.1', 'MinC', numerator=50, denominator=100, penalty_points=300.0
    )
    mtur = _summary(
        'INMS 1.1', 'MTur', numerator=50, denominator=100, penalty_points=150.0
    )
...
def test_multi_asset_pairs_are_consolidated_independently_per_asset() -> None:
    ...
    wifi_minc = _summary(
        'INMS 1.1', 'MinC', asset='WI-FI', numerator=50, denominator=100
    )
    wifi_mtur = _summary(
        'INMS 1.1', 'MTur', asset='WI-FI', numerator=50, denominator=100
    )

    rows = with_orgao_consolidation(
        [file_server_minc, file_server_mtur, wifi_minc, wifi_mtur]
    )
